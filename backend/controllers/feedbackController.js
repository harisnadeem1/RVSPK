...
export const sendFeedbackEmail = async (req, res) => {
  const { name, cnic, mobile, email, subject, message } = req.body
  ...
  const htmlBody = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #c0392b; padding: 24px; border-radius: 8px 8px 0 0;">
        <h2 style="color: #ffffff; margin: 0;">New Feedback Submission</h2>
        ...
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee;">
              <b style="color: #555;">CNIC</b>
            </td>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee; color: #222;">${cnic || 'Not provided'}</td>
          </tr>
          <tr>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee;">
              <b style="color: #555;">Mobile</b>
            </td>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee; color: #222;">${mobile || 'Not provided'}</td>
          </tr>
          <tr>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee;">
      ...
      <div style="background: #eee; padding: 14px 24px; border-radius: 0 0 8px 8px; text-align: center;">
        <p style="margin: 0; font-size: 12px; color: #888;">
          Sent from rvspk.com feedback form · Right Vision Securities
        </p>
      </div>
    </div>
  `
  ...
  try {
    await transporter.sendMail({
      from: `"RVSPL Feedback Form" <${process.env.GMAIL_USER}>`,
      to: process.env.FEEDBACK_RECEIVER,
      replyTo: email,
      subject: `[RVSPL Feedback] ${subject}`,
      html: htmlBody,
    })
