...
export const submitGrievance = async (req, res) => {
  const {
    ...
    accountType, address,
    grievanceType, grievanceAgainst,
    tradingAccount, details
  } = req.body

  if (!name || !cnic || !cell || !email || !grievanceType || !details) {
    return res.status(400).json({
      success: false,
      error: 'Name, CNIC, cell, email, grievance type, and details are required.'
    })
  }
  ...
  const htmlBody = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #c0392b; padding: 24px; border-radius: 8px 8px 0 0;">
        <h2 style="color: #ffffff; margin: 0;">New Client Grievance</h2>
      ...
      <div style="background: #f9f9f9; padding: 24px; border: 1px solid #e0e0e0;">
        ...
        <p style="margin: 0 0 12px; font-weight: bold; color: #c0392b; font-size: 13px; text-transform: uppercase; letter-spacing: 0.05em;">Grievance Details</p>
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee; width: 160px;"><b style="color: #555;">Grievance Type</b></td>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee; color: #222;">${grievanceType}</td>
          </tr>
          <tr>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee;"><b style="color: #555;">Grievance Against</b></td>
            <td style="padding: 10px 0; border-bottom: 1px solid #eee; color: #222;">${grievanceAgainst || 'Not specified'}</td>
          </tr>
          <tr>
      ...
      <div style="background: #eee; padding: 14px 24px; border-radius: 0 0 8px 8px; text-align: center;">
        <p style="margin: 0; font-size: 12px; color: #888;">
          Sent from rvspk.com grievance form · Right Vision Securities
        </p>
      </div>
    </div>
  `
  ...
  try {
    await transporter.sendMail({
      from: `"RVSPL Grievance Form" <${process.env.GMAIL_USER}>`,
      to: process.env.GRIEVANCE_RECEIVER,
      replyTo: email,
      subject: `[RVSPL Grievance] ${grievanceType}`,
      html: htmlBody,
    ...
    return res.status(200).json({ success: true, message: 'Grievance submitted successfully.' })
  } catch (error) {
    console.error('Grievance email error:', error)
    return res.status(500).json({ success: false, error: 'Failed to submit grievance. Please try again.' })
  }
}