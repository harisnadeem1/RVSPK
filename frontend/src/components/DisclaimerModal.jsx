...
function DisclaimerModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div
        className="relative bg-card rounded-2xl shadow-2xl w-full max-w-4xl mx-auto max-h-[90vh] flex flex-col overflow-hidden"
        role="dialog"
        aria-modal="true"
        ...
        <div className="flex items-center gap-2 px-5 pt-5 pb-3 border-b border-border">
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center gap-2">
              ...
              {s < 2 && (
                <div
                  className={`h-0.5 w-10 rounded transition-all duration-300 ${step > s ? 'bg-accent' : 'bg-muted'
                    }`}
                />
              )}
            </div>
          ...
          <span className="ml-2 text-xs text-muted-foreground">
            {step === 1 ? 'SECP & PMEX' : 'Website Disclaimer'}
          </span>
        </div>
  ...
  <div className="flex flex-col flex-1 overflow-hidden">
    <div className="flex-1 overflow-y-auto px-4 sm:px-5 md:px-8 py-5 space-y-5">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6 items-stretch">
        {/* Image 1 */}
        <div className="relative bg-white rounded-2xl border border-border p-3 sm:p-4 flex items-center justify-center min-h-[420px] sm:min-h-[520px]">
          {!imgLoaded && (
          ...
          <img
            ...
            alt="Right Vision Securities Disclaimer 1"
        ...
        <div className="relative bg-white rounded-2xl border border-border p-3 sm:p-4 flex items-center justify-center min-h-[420px] sm:min-h-[520px]">
          <img
            src="/documents/pmex_disclamir.jpeg"
            alt="Right Vision Securities Disclaimer 2"
            className="w-full h-full max-h-[78vh] object-contain rounded-lg"
          />
        </div>
      </div>
    ...
    <div className="border-t border-border bg-card px-4 sm:px-5 md:px-8 py-4 flex flex-col sm:flex-row gap-3">
      <Button
        onClick={() => setStep(2)}
